(function(){
  ...
  function startLocal(scope){
    ...
    const title=scope==='anatomy'?'Clinical Anatomy':scope==='physiology'?'Clinical Physiology':'Anatomy & Physiology';
    window.UKMLA_QUESTION_BANK?.storeSet({schemaVersion:'ukmla-local-biomedical-v1',quizId:quiz.id,topic:title,generatedAt:new Date().toISOString(),sourceType:'biomedical',questions},{sourceType:'biomedical',title});
    drawQuestion();
  }
  ...
  function drawResult(){
    ...
    container.innerHTML=`<article class="quiz-card" style="max-width:760px;margin:auto;text-align:center"><div class="eyebrow">Biomedical question set complete</div><div class="boast-number" style="margin:28px 0">${quiz.correct}/${quiz.questions.length}</div><h2>${percent}%</h2><p>Clinical Anatomy, Clinical Physiology, Question Bank history and analytics have been updated.</p><div class="card-actions" style="justify-content:center"><button class="btn primary" id="biomedical-again">Another question set</button><button class="btn" id="biomedical-analytics">Open analytics</button></div></article>`;
