(function(){
  ...
  const RELEVANCE_CLASSES=[
    'correct_answer',
    'close_mimic',
    'same_presentation_alternative',
    'same_pathway_wrong_stage',
    'same_treatment_class',
    'same_investigation_family',
    'same_contraindication_family',
    'same_escalation_scale',
    'same_disposition_scale'
  ];

  const DISTRACTOR_RULES=`\n\nHARD DISTRACTOR-VALIDITY LOCK — MANDATORY FOR EVERY QUESTION:\nEvery distractor must be a genuine clinical competitor for this exact patient, presentation, body system, decision point and answer category. An option is invalid if a competent final-year medical student could dismiss it immediately because it concerns an unrelated organ system, unrelated procedure, impossible demographic, wrong care setting or wholly irrelevant pathway.\n\nApply these rules:\n1. SAME CLINICAL FRAME: Define one hidden clinicalDomain and one hidden decisionFrame for the question. All five options must plausibly belong to that frame. Cross-system options are allowed only when they are genuine differentials or competing actions for the same presentation, such as acute coronary syndrome versus pulmonary embolism in chest pain.\n2. REAL COMPETITORS: At least three of the four distractors must remain genuinely plausible until the decisive clue, timing, contraindication, severity threshold or pathway stage is applied. The fourth may be less likely but must still be clinically relevant.\n3. NO ABSURD DECOYS: Do not use an unrelated examination, investigation, treatment or diagnosis merely because it matches the grammatical category. For example, digital rectal examination is not a valid distractor against bronchoscopy in a chronic respiratory investigation question unless the vignette genuinely creates a colorectal indication.\n4. MATCH THE DECISION POINT:\n- diagnosis questions: use close differentials for the same presentation and demographic;\n- investigation questions: use tests a clinician might reasonably consider for that presentation at that stage;\n- treatment questions: use plausible treatments for the same condition or close mimic at the same severity;\n- contraindication questions: use real potential contraindicating factors for the actual proposed treatment/pathway;\n- emergency-management questions: use competing immediate actions in the same emergency;\n- escalation/disposition questions: use realistic alternatives along the same urgency or destination scale.\n5. SAME LEVEL OF ABSTRACTION: Do not mix a broad strategy with a specific procedure or a general diagnosis with a microscopic subtype unless the lead-in demands that level.\n6. HIDDEN JUSTIFICATION: Each option must include hidden clinicalDomain, relevanceClass and plausibilityReason metadata. For the correct option use relevanceClass = correct_answer. For every distractor, plausibilityReason must state briefly why a knowledgeable candidate might consider it before applying the decisive clue.\n7. REBUILD INVALID ITEMS: If even one distractor is clinically irrelevant, replace that distractor. If fewer than three strong competitors can be created, rebuild the entire question rather than padding the set with nonsense.`;

  const REVIEW_SYSTEM=`You are the final distractor-validity editor for a very difficult UKMLA SBA set. Return the complete ten-question set in exactly the supplied JSON schema and nothing else. Review each option as a senior UK medical educator. Category matching alone is not enough. A distractor must be clinically credible for the exact vignette, organ system, demographic, care setting and pathway stage.\n\nReject and replace:\n- unrelated-organ-system options;\n- investigations that would never be considered for the presentation;\n- treatments for unrelated diseases;\n- diagnoses outside a credible differential;\n- contraindications unrelated to the proposed standard pathway;\n- actions at an impossible or irrelevant stage;\n- absurd decoys that make the answer obvious.\n\nAt least three distractors per item must be strong competitors. Preserve the target condition, clinical anchor, answer category, question type, correct clinical answer, concise wording, hidden scoring metadata and correct-answer mapping. Rewrite distractors, lead-in or the whole item when needed. Keep visible answer options short. Hidden plausibilityReason text may be longer but must remain concise and specific.`;
  ...
  async function applyGate(response,body,headers){
    ...
    emit('Running the clinical distractor-validity checkpoint on all ten questions…',initial.issues);
    ...
    if(bestAudit.issues.length===0) emit('Distractor-validity checkpoint passed: all options are credible within the clinical frame.');
